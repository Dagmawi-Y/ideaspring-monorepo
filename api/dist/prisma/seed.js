...
async function main() {
    ...
    console.log('Investor roles created successfully.');
    await prisma.taxRelief.createMany({
        data: [{ relief_name: 'SEIS' }, { relief_name: 'EIS' }],
    });
    console.log('Tax relief options created successfully.');
    ...
    console.log('Startup stages created successfully.');
    ...
    const createdPermissions = await Promise.all(permissionNames.map((permissionName) => prisma.permission.create({ data: { name: permissionName } })));
    ...
    const entrepreneurRoleId = (await prisma.role.findUnique({ where: { name: Role.Entrepreneur } })).id;
    rolePermissionData.push(...entrepreneurPermissions.map((permission) => ({
    ...
    const investorRoleId = (await prisma.role.findUnique({ where: { name: Role.Investor } })).id;
    rolePermissionData.push(...investorPermissions.map((permission) => ({
    ...
    const engagerRoleId = (await prisma.role.findUnique({ where: { name: Role.Engager } })).id;
    rolePermissionData.push(...engagerPermissions.map((permission) => ({
        roleId: engagerRoleId,
        permissionId: permission.id,
    })));
    const adminRoleId = (await prisma.role.findUnique({ where: { name: Role.Admin } })).id;
    rolePermissionData.push(...createdPermissions.map((permission) => ({
        roleId: adminRoleId,
        permissionId: permission.id,
    })));
    ...
    for (const user of defaultUsers) {
        ...
        console.log(`User created with email: ${createdUser.email} and role: ${user.role}`);
    }
